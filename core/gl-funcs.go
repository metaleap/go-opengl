...
// int cancallDebugMessageCallback() { return ((ptrglDebugMessageCallback == NULL) ? 0 : 1); }
...
//	Returns whether the current GL context supports calling the DebugMessageCallback() function.
func (_ GlSupport) DebugMessageCallback() bool {
	return C.cancallDebugMessageCallback() == 1
}

//	If Supports.DebugMessageCallback() is true, calls DebugMessageCallback() and yields the err returned by Util.Error(), if any.
func (_ GlTry) DebugMessageCallback(callback Ptr, userParam Ptr) (err error) {
	if !Supports.DebugMessageCallback() {
		err = fmt.Errorf("DebugMessageCallback() function call not supported by current GL context.")
	} else {
		DebugMessageCallback(callback, userParam)
		err = Util.Error("DebugMessageCallback()")
	}
	return
}
